...
import { useFileStore, FileStoreState } from '../store/fileStore'; // Keep this for file state

// Define the expected structure of the data received from SSE
interface FileProgressData {
  fileId: string;
  progress: number; // Should be percentage (0-100)
  status: string; // Should match FileStatus enum values
}
...
export function useFileProgressSSE() {
  ...
  const updateFileProgressInStore = useFileStore((state: FileStoreState) => state.updateFileProgress);
  ...
  useEffect(() => {
    ...
    es.addEventListener('fileProgressUpdate', (event) => {
      try {
        const data: FileProgressData = JSON.parse(event.data);
