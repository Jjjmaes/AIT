...
import { getAIConfigById, updateAIConfig, AIConfigPayload } from '../api/aiConfigService';
...
const AIConfigEditPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      setError(null);
      try {
        const response = await getAIConfigById(configId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: Partial<AIConfigPayload> = {
    ...
    try {
      const response = await updateAIConfig(configId!, payload);
      if (response.success && response.data?.config) {
        navigate('/ai-configs');
      } else {
  ...
  if (isLoading) {
    return (
      <div style={{ 
        ...
        <div>
          <p style={{ fontSize: '1.1rem' }}>正在加载 AI 配置...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="edit-ai-config-page" style={{ maxWidth: '900px', margin: '0 auto', padding: '1rem' }}>
      <div style={{ 
        display: 'flex', 
        ...
        <h1 style={{ margin: 0, color: '#333' }}>编辑 AI 配置</h1>
        <button 
          type="button" 
          ...
          <div className="card-content" style={{ padding: '1.5rem' }}>
            ...
            <div className="form-group" style={{ marginBottom: '1.25rem' }}>
              ...
              <input
                ...
                required
                disabled={isSaving}
                placeholder={apiKeyChanged ? "输入新的 API 密钥" : "••••••••••••••••••••••••"}
                style={{ 
                  width: '100%',
                  padding: '0.75rem',
                  borderRadius: '4px',
...
export default AIConfigEditPage; 