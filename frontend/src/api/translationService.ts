...
import { axiosInstance as client } from './base'; // Import and alias the correct instance
// TODO: Adjust the path and content of this import based on your actual type definitions
// Example: import { TranslationTask, TaskPriority, TaskStatus } from '../../types/tasks'; 
// Or define simplified types here if needed for now.
type TaskStatus = 
  | 'pending' 
  | 'preprocessing' 
...
export interface TranslationTask { // Define based on usage in TranslationCenterPage
  _id: string;
  projectId: string;
  fileName: string;
  projectName: string;
  ...
  progress?: number;
  assignedTo?: string;
  createdAt?: string; // Added based on mock data generation
}

// Interface for the statistics object returned by the API or calculated client-side
export interface TaskStats {
  total: number;
  pending: number;
  inProgress: number;
  review?: number; // Optional, depending on your workflow
  completed: number;
  revisionNeeded?: number; // Added based on usage in TranslationCenterPage stats card
  // Add other relevant stats if needed
  preprocessing?: number;
  ready_for_translation?: number;
  in_translation_queue?: number;
  translating?: number;
  translation_failed?: number;
  translated_pending_confirmation?: number;
  translation_confirmed?: number;
...
const generateMockTasks = (count = 25): TranslationTask[] => {
  const tasks: TranslationTask[] = [];
  // TODO: Ensure these statuses match your backend definitions
  const statuses: TaskStatus[] = [ // Use only NEW valid statuses for mock generation
    'pending', 
    'preprocessing', 
    'ready_for_translation', 
    'in_translation_queue', 
 ...
 */
export const getTranslationTasks = async (): Promise<{ tasks: TranslationTask[], stats: TaskStats }> => {
  ...
  try {
    ...
    const stats: TaskStats = mockTasks.reduce<TaskStats>((acc, task) => {
      acc.total++;
      switch (task.status) {
        case 'pending': acc.pending++; break;
        case 'completed': acc.completed++; break;
        case 'preprocessing': acc.preprocessing = (acc.preprocessing || 0) + 1; break;
        case 'ready_for_translation': acc.ready_for_translation = (acc.ready_for_translation || 0) + 1; break;
        case 'in_translation_queue': acc.in_translation_queue = (acc.in_translation_queue || 0) + 1; break;
        case 'translating': acc.translating = (acc.translating || 0) + 1; break;
        case 'translation_failed': acc.translation_failed = (acc.translation_failed || 0) + 1; break;
        case 'translated_pending_confirmation': acc.translated_pending_confirmation = (acc.translated_pending_confirmation || 0) + 1; break;
        case 'translation_confirmed': acc.translation_confirmed = (acc.translation_confirmed || 0) + 1; break;
        case 'in_review_queue': acc.in_review_queue = (acc.in_review_queue || 0) + 1; break;
        case 'reviewing': acc.reviewing = (acc.reviewing || 0) + 1; break;
        case 'review_failed': acc.review_failed = (acc.review_failed || 0) + 1; break;
        case 'reviewed_pending_confirmation': acc.reviewed_pending_confirmation = (acc.reviewed_pending_confirmation || 0) + 1; break;
      }
      return acc;
    }, { 
 ...
 */
export const retryTranslationTask = async (taskId: string): Promise<void> => {
  try {
    // Adjust method (POST, PUT) and endpoint as needed
    await client.post(`/api/translations/tasks/${taskId}/retry`);
...
// Add other translation-related API functions here if needed (e.g., getTaskById, updateTaskStatus)

// --- NEW: Segment Types --- 
export type SegmentStatus = 'unconfirmed' | 'confirmed' | 'needs_revision';

export interface Segment {
  _id: string; // Unique segment ID
  taskId: string; // ID of the parent task
  sequence: number; // Order of the segment in the document
  source: string; // Source text
  target: string; // Target text (editable)
  status: SegmentStatus;
  // Add other relevant fields like TM match percentage, quality score, etc. if needed
}
 ...
 */
const generateMockSegments = (taskId: string, count = 10): Segment[] => {
    const segments: Segment[] = [];
    const statuses: SegmentStatus[] = ['unconfirmed', 'confirmed', 'unconfirmed']; // Skew towards unconfirmed
    for (let i = 1; i <= count; i++) {
 ...
 * TODO: Replace mock data with actual API call.
 */
export const getTranslatedSegments = async (taskId: string): Promise<Segment[]> => {
 ...
 */
export const updateSegment = async (segmentId: string, updatedData: Partial<Pick<Segment, 'target' | 'status'>>): Promise<Segment> => {
    ...
    const updatedMockSegment: Partial<Segment> = { 
        _id: segmentId, 
    ...
    return updatedMockSegment as Segment; 
    // --- End of mock data --- 
};
 ...
 * TODO: Replace mock logic with actual API call.
 */
export const confirmTranslationTask = async (taskId: string): Promise<void> => {
