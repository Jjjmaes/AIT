...
describe('Review Service', () => {
    ...
    let mockAIReviewService;
    let customReviewService;
    // 声明mock方法
    let mockStartAIReview;
    let mockCompleteSegmentReview;
    let mockGetSegmentReviewResult;
    let mockAddSegmentIssue;
    let mockResolveSegmentIssue;
    let mockBatchUpdateSegmentStatus;
    beforeEach(() => {
        jest.clearAllMocks();
        // Setup MongoDB ObjectId mock
        jest.spyOn(mongoose_1.default.Types, 'ObjectId').mockImplementation((id) => id);
        ...
        customReviewService = new review_service_1.ReviewService(mockAIReviewService);
        // 初始化mock方法
    ...
    describe('startAIReview', () => {
        it('should throw NotFoundError if segment does not exist', async () => {
            ...
            segment_model_1.Segment.findById.mockImplementation(() => {
                return {
            ...
            await expect(customReviewService.startAIReview(mockSegmentId, mockUserId)).rejects.toThrow(expect.objectContaining({ name: 'NotFoundError' }));
        });
        it('should throw BadRequestError if segment is not in TRANSLATED status', async () => {
            ...
            await expect(customReviewService.startAIReview(mockSegmentId, mockUserId)).rejects.toThrow(expect.objectContaining({ name: 'BadRequestError' }));
        });
        it('should throw ForbiddenError if user has no permission', async () => {
            mockStartAIReview.mockImplementationOnce(async () => {
                throw new ForbiddenError('用户无权审校段落');
            });
            await expect(customReviewService.startAIReview(mockSegmentId, unauthorizedUserId)).rejects.toThrow(expect.objectContaining({ name: 'ForbiddenError' }));
        });
        it('should update segment status to REVIEW_IN_PROGRESS and save review result', async () => {
            // Expected result
            const expectedSegment = {
                ...mockSegment,
                status: segment_model_1.SegmentStatus.REVIEW_IN_PROGRESS,
                reviewer: mockUserId,
                reviewResult: {
            ...
            customReviewService.startAIReview.mockResolvedValueOnce(expectedSegment);
            // Act
            const result = await customReviewService.startAIReview(mockSegmentId, mockUserId);
            // Assert
            expect(result.status).toBe(segment_model_1.SegmentStatus.REVIEW_IN_PROGRESS);
            expect(result.reviewResult).toBeDefined();
        });
        it('should throw BadRequestError if segmentId is empty', async () => {
            // 直接调用service方法，不需要mock
            await expect(customReviewService.startAIReview('', mockUserId)).rejects.toThrow(expect.objectContaining({ name: 'BadRequestError' }));
        });
        it('should throw BadRequestError if userId is empty', async () => {
            // 直接调用service方法，不需要mock
            await expect(customReviewService.startAIReview(mockSegmentId, '')).rejects.toThrow(expect.objectContaining({ name: 'BadRequestError' }));
        });
        it('should throw BadRequestError if segment has no translation', async () => {
            ...
            file_model_1.File.findById.mockImplementation(() => {
                return { exec: jest.fn().mockResolvedValue(mockFile) };
            });
            project_model_1.default.findById.mockImplementation(() => {
                return { exec: jest.fn().mockResolvedValue(mockProject) };
            });
            await expect(customReviewService.startAIReview(mockSegmentId, mockUserId)).rejects.toThrow(expect.objectContaining({
        ...
        it('should handle missing AI review service gracefully', async () => {
            ...
            const serviceWithoutAI = new review_service_1.ReviewService();
            ...
            ai_adapters_1.AIServiceFactory.getInstance.mockReturnValue(undefined);
            await expect(serviceWithoutAI.startAIReview(mockSegmentId, mockUserId)).rejects.toThrow(new BadRequestError('开始AI审校失败: Cannot read properties of undefined (reading \'getReviewAdapter\')'));
        });
        it('should handle custom review prompt template', async () => {
            ...
            expect(mockAIReviewService.reviewTranslation).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('completeSegmentReview', () => {
        it('should throw NotFoundError if segment does not exist', async () => {
            ...
            // 直接调用service方法，使用更灵活的错误匹配方式
            await expect(customReviewService.completeSegmentReview(mockSegmentId, mockUserId, { finalTranslation: 'Updated translation' })).rejects.toThrow(expect.objectContaining({ name: 'NotFoundError' }));
        });
        it('should throw BadRequestError if segment is not in review status', async () => {
            ...
            await expect(customReviewService.completeSegmentReview(mockSegmentId, mockUserId, { finalTranslation: 'Updated translation' })).rejects.toThrow(expect.objectContaining({ name: 'BadRequestError' }));
        });
        it('should throw ForbiddenError if user has no permission', async () => {
            mockCompleteSegmentReview.mockImplementationOnce(async () => {
                throw new ForbiddenError('用户无权完成审校');
            });
            await expect(customReviewService.completeSegmentReview(mockSegmentId, unauthorizedUserId, { finalTranslation: 'Updated translation' })).rejects.toThrow(expect.objectContaining({ name: 'ForbiddenError' }));
        });
        it('should update segment with review result and set status to REVIEW_COMPLETED', async () => {
            ...
            const expectedSegment = {
                ...mockSegment,
                status: segment_model_1.SegmentStatus.REVIEW_COMPLETED,
            ...
            // Mock the implementation
            customReviewService.completeSegmentReview.mockResolvedValueOnce(expectedSegment);
            // Act
            const result = await customReviewService.completeSegmentReview(mockSegmentId, mockUserId, reviewData);
            // Assert
            expect(result.status).toBe(segment_model_1.SegmentStatus.REVIEW_COMPLETED);
        ...
        it('should throw BadRequestError if reviewData is empty', async () => {
            // 直接调用service方法，不需要mock
            await expect(customReviewService.completeSegmentReview(mockSegmentId, mockUserId, null)).rejects.toThrow(expect.objectContaining({
                name: 'BadRequestError',
        ...
        it('should handle invalid review status', async () => {
            ...
            await expect(customReviewService.completeSegmentReview(mockSegmentId, mockUserId, { status: 'INVALID_STATUS' })).rejects.toThrow(expect.objectContaining({
                name: 'BadRequestError',
        ...
        it('should use suggested translation when final translation is not provided', async () => {
            ...
            const result = await customReviewService.completeSegmentReview(mockSegmentId, mockUserId, { acceptedChanges: true });
        ...
        it('should handle review scores update', async () => {
            const reviewScores = [
                { type: segment_model_1.ReviewScoreType.ACCURACY, score: 0.9, details: 'Very accurate' },
                { type: segment_model_1.ReviewScoreType.FLUENCY, score: 0.8, details: 'Quite fluent' }
            ];
            // Mock segment in review
            const segmentInReview = {
                ...mockSegment,
            ...
            const result = await customReviewService.completeSegmentReview(mockSegmentId, mockUserId, {
                finalTranslation: 'Final translation',
                scores: reviewScores
            });
        ...
        it('should return complete review history', async () => {
            const mockReviewHistory = [
                {
                    ...
                    modifiedBy: new mongoose_1.default.Types.ObjectId('user123'),
                    aiGenerated: true,
            ...
            const mockReviewResult = {
                ...
                reviewerId: new mongoose_1.default.Types.ObjectId('user123'),
                aiReviewer: 'AI',
                modificationDegree: 0,
                acceptedChanges: true
            };
            const segmentInReview = {
                _id: new mongoose_1.default.Types.ObjectId('segment123'),
                fileId: new mongoose_1.default.Types.ObjectId('file123'),
                content: 'Original content',
                translation: 'Second version',
                status: segment_model_1.SegmentStatus.REVIEW_IN_PROGRESS,
                reviewer: new mongoose_1.default.Types.ObjectId('user123'),
    ...
    describe('addSegmentIssue', () => {
        it('should add a new issue to the segment', async () => {
            ...
            // Mock the implementation
            customReviewService.addSegmentIssue.mockResolvedValueOnce(expectedIssue);
            // Act
            const result = await customReviewService.addSegmentIssue(mockSegmentId, mockUserId, issueData);
            // Assert
    ...
    describe('resolveSegmentIssue', () => {
        it('should resolve an existing issue', async () => {
            ...
            // Mock the implementation
            customReviewService.resolveSegmentIssue.mockResolvedValueOnce(resolvedIssue);
            // Act
            const result = await customReviewService.resolveSegmentIssue(mockSegmentId, 'issue123', mockUserId);
            // Assert
    ...
    describe('batchUpdateSegmentStatus', () => {
        it('should update multiple segments with the given status', async () => {
            ...
            const newStatus = segment_model_1.SegmentStatus.REVIEW_COMPLETED;
            // Expected result
            ...
            // Mock the implementation
            customReviewService.batchUpdateSegmentStatus.mockResolvedValueOnce(expectedResult);
            // Act
            const result = await customReviewService.batchUpdateSegmentStatus(segmentIds, mockUserId, newStatus);
            // Assert
    ...
    describe('getSegmentReviewResult', () => {
        it('should throw NotFoundError if segment does not exist', async () => {
            ...
            // 直接调用service方法，使用更灵活的错误匹配方式
            await expect(customReviewService.getSegmentReviewResult(mockSegmentId, mockUserId)).rejects.toThrow(expect.objectContaining({ name: 'NotFoundError' }));
        });
        ...
        it('should throw ForbiddenError if user has no permission', async () => {
            ...
            await expect(customReviewService.getSegmentReviewResult(mockSegmentId, unauthorizedUserId)).rejects.toThrow(expect.objectContaining({ name: 'ForbiddenError' }));
        });
        it('should return review result for project manager', async () => {
            ...
            const mockIssues = [{ type: segment_model_1.IssueType.GRAMMAR, description: '语法问题' }];
            ...
            // Mock the implementation
            customReviewService.getSegmentReviewResult.mockResolvedValueOnce(expectedResult);
            // Act
            const result = await customReviewService.getSegmentReviewResult(mockSegmentId, mockUserId);
            // Assert
        ...
        it('should return complete review history', async () => {
            ...
            const segmentWithHistory = {
                ...
                status: segment_model_1.SegmentStatus.REVIEW_COMPLETED,
                reviewer: new mongoose_1.default.Types.ObjectId('user123'),
                translator: new mongoose_1.default.Types.ObjectId('translator123'),
                reviewHistory: mockReviewHistory,
                reviewResult: mockReviewResult,
                issues: []
            };
