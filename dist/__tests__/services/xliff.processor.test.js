...
const xliff_processor_1 = require("../../services/fileProcessing/processors/xliff.processor");
...
describe('XliffProcessor', () => {
    let processor;
    let mockReadFile;
    let mockWriteFile; // Use a general mock type
    beforeEach(() => {
        processor = new xliff_processor_1.XliffProcessor();
        mockReadFile = promises_1.readFile;
        // Explicitly type mockWriteFile
        mockWriteFile = promises_1.writeFile;
    });
    afterEach(() => {
    ...
    describe('extractSegments', () => {
        ...
        test('should correctly parse a MemoQ XLIFF file', async () => {
            ...
            expect(result.segments[0]).toMatchObject({
                ...
                status: segment_model_1.SegmentStatus.TRANSLATED, // Maps from 'Translated' m:state and 'translated' state
                metadata: {
                    xliffId: '1',
            ...
            expect(result.segments[1]).toMatchObject({
                ...
                status: segment_model_1.SegmentStatus.CONFIRMED, // Maps from 'final' target state
                metadata: {
                    xliffId: '2',
        ...
        test('should throw an error if readFile fails', async () => {
            ...
            expect(logger_1.default.error).toHaveBeenCalledWith(`Error processing XLIFF file ${filePath}:`, readError);
        });
        test('should throw an error for invalid XML content', async () => {
            ...
            expect(logger_1.default.error).toHaveBeenCalledWith(expect.stringContaining(`Error processing XLIFF file ${filePath}`), expect.any(Error));
        });
        test('should return empty results for a file with no trans-units', async () => {
            ...
            expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining(`No trans-unit elements found or xpath returned empty array in ${filePath}`), [] // Expect an empty array as the second argument now
            );
        });
    });
    describe('writeTranslations', () => {
        ...
        test('should correctly write translations to a standard XLIFF file', async () => {
            ...
            await processor.writeTranslations(mockSegments, originalFilePath, targetFilePath);
            ...
            const writtenContent = writeArgs[1] ?? '';
            ...
            const warnMock = logger_1.default.warn;
        ...
        test('should correctly write translations to a MemoQ XLIFF file', async () => {
            const mockSegments = [
                ...
                {
                    _id: new mongoose_1.Types.ObjectId(), index: 0, sourceText: 'Source text 1',
                    finalText: 'Zieltext 1 Final', status: segment_model_1.SegmentStatus.CONFIRMED,
                    metadata: { xliffId: '1' }
                },
                // Segment 2: Status REVIEW_COMPLETED -> m:state="Confirmed", state="reviewed"
                {
                    _id: new mongoose_1.Types.ObjectId(), index: 1, sourceText: 'Source text 2',
                    finalText: 'Zieltext 2 Reviewed', status: segment_model_1.SegmentStatus.REVIEW_COMPLETED,
                    metadata: { xliffId: '2' }
                },
                // Segment 3: Status TRANSLATED -> m:state="Translated", state="translated" (New target)
                {
                    _id: new mongoose_1.Types.ObjectId(), index: 2, sourceText: 'Source text 3',
                    finalText: 'Zieltext 3 Translated', status: segment_model_1.SegmentStatus.TRANSLATED,
                    metadata: { xliffId: '3' }
                },
                // Segment with missing xliffId (should be skipped)
                {
                    _id: new mongoose_1.Types.ObjectId(), index: 3, sourceText: 'Skipped',
                    finalText: 'Skipped', status: segment_model_1.SegmentStatus.CONFIRMED,
                    metadata: {}
                },
            ];
            ...
            await processor.writeTranslations(mockSegments, originalFilePath, targetFilePath, { isMemoQ: true });
            expect(mockReadFile).toHaveBeenCalledWith(originalFilePath, 'utf-8');
            expect(mockWriteFile).toHaveBeenCalledTimes(1);
            // Get written content
            expect(mockWriteFile.mock.calls.length).toBeGreaterThan(0);
        ...
        test('should throw an error if readFile fails', async () => {
            ...
            const mockSegments = [
                ...
                { _id: new mongoose_1.Types.ObjectId(), index: 0, metadata: { xliffId: '1' } }
            ];
            ...
            expect(logger_1.default.error).toHaveBeenCalledWith(`Error processing XLIFF file ${originalFilePath}:`, readError);
        ...
        test('should throw an error if writeFile fails', async () => {
            ...
            // Use unknown cast for minimal data
            mockWriteFile.mockRejectedValue(writeError); // Cast to never to satisfy mock
            ...
            const mockSegments = [
                ...
                {
                    _id: new mongoose_1.Types.ObjectId(), index: 0, sourceText: 'Hello World',
                    finalText: 'Bonjour Final', status: segment_model_1.SegmentStatus.CONFIRMED,
                    metadata: { xliffId: '1' }
                },
            ];
            await expect(processor.writeTranslations(mockSegments, originalFilePath, targetFilePath))
            ...
            expect(logger_1.default.error).toHaveBeenCalledWith(`Error processing XLIFF file ${originalFilePath}:`, writeError);
        });
