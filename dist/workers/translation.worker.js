...
const reviewQueue_service_1 = require("../services/reviewQueue.service");
...
async function processJob(job) {
    ...
    try {
        ...
        for (const { segmentId, fileId: currentFileId } of segmentsToTranslate) {
            let updatedSegment = null; // Variable to hold result
            try {
                // Pass requesterRoles to translateSegment call in the correct order
                updatedSegment = await translation_service_1.translationService.translateSegment(segmentId, userId, requesterRoles, options);
                processedSegments++;
            }
            catch (error) {
            ...
            if (updatedSegment && updatedSegment.status === segment_model_1.SegmentStatus.TRANSLATED) {
                try {
                    // Restore adding job to review queue
                    const reviewJobId = await reviewQueue_service_1.reviewQueueService.addSegmentReviewJob(segmentId, userId, requesterRoles // Pass roles here
                    );
                    if (reviewJobId) {
                        logger_1.default.info(`Job ${job.id}: Added segment ${segmentId} to AI review queue (Job ID: ${reviewJobId}).`);
                    }
                }
                catch (queueError) {
                    logger_1.default.error(`Job ${job.id}: Failed to add segment ${segmentId} to AI review queue: ${queueError.message}`, queueError);
                    // Do not fail the translation job if queuing review fails
                }
            }
            else if (updatedSegment && updatedSegment.status === segment_model_1.SegmentStatus.TRANSLATED_TM) {
                logger_1.default.info(`Job ${job.id}: Segment ${segmentId} was translated by TM, skipping AI review trigger.`);
                ...
                logger_1.default.warn(`Job ${job.id}: Segment ${segmentId} not queued for review (Status: ${updatedSegment?.status || 'Update Failed'}).`);
            }
            // ---------------------------------------------------
