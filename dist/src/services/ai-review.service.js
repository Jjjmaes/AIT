    ...
    async reviewTranslation(original, translation, options // Use the defined interface
        ...
        try {
            ...
            if (!effectivePrompt && options.promptTemplateId) {
                ...
                try {
                    const template = await this.promptTemplateService.getPromptTemplateById(options.promptTemplateId);
            ...
            logger_1.default.debug(`[${methodName}] Executing adapter review.`);
            const reviewResult = await reviewAdapter.reviewText(reviewOptions);
            logger_1.default.debug(`[${methodName}] Review process completed successfully.`);
            return reviewResult;
        }
        catch (error) {
            logger_1.default.error(`[${methodName}] Error during review process:`, error);
