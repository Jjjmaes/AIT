...
const app_1 = require("../app"); // Adjust path as needed!
...
class ProjectService {
     ...
     * 获取项目文件列表
     */
    async getProjectFiles(projectId, userId, requesterRoles) {
        const methodName = 'getProjectFiles';
        logger_1.default.debug(`[Service/${methodName}] ENTER - ProjectId: ${projectId}, UserId: ${userId}, Roles: ${JSON.stringify(requesterRoles)}`); // Log entry
        (0, errorHandler_1.validateId)(projectId, '项目');
        (0, errorHandler_1.validateId)(userId, '用户');
        try {
            // Pass requesterRoles to getProjectById for proper access validation
            logger_1.default.debug(`[Service/${methodName}] Calling getProjectById...`);
            const project = await this.getProjectById(projectId, userId, requesterRoles);
            logger_1.default.debug(`[Service/${methodName}] getProjectById returned project: ${project?._id}`);
            logger_1.default.debug(`[Service/${methodName}] Calling File.find({ projectId: ${project._id} })...`);
            const files = await file_model_1.File.find({ projectId: project._id }).sort({ createdAt: -1 }).exec();
            logger_1.default.debug(`[Service/${methodName}] File.find returned ${files.length} files.`);
            return files;
        }
        catch (error) {
            logger_1.default.error(`[Service/${methodName}] FAILED - ProjectId: ${projectId}, UserId: ${userId}. Error:`, error);
    ...
    async updateFileProgress(fileId, userId) {
        ...
        try {
            ...
            const translatedOkSegments = await segment_model_1.Segment.countDocuments({
                fileId: file._id,
            ...
            const reviewedOkSegments = await segment_model_1.Segment.countDocuments({
                fileId: file._id,
                // CHECK ENUM: Use the correct status for 'Reviewed' or 'Completed' state
                status: segment_model_1.SegmentStatus.REVIEW_COMPLETED // Assuming REVIEW_COMPLETED exists
            });
            const failedSegments = await segment_model_1.Segment.countDocuments({
                fileId: file._id,
            ...
            if (allProcessedCount >= totalSegments && totalSegments > 0) {
                if (failedSegments > 0) {
                    finalStatus = file_model_1.FileStatus.ERROR;
                    ...
                    finalStatus = file_model_1.FileStatus.TRANSLATED; // Set to TRANSLATED when 100% translated
                    // Check if all are actually REVIEWED to set COMPLETED
                    if (completedCount === totalSegments) {
                        finalStatus = file_model_1.FileStatus.COMPLETED; // Or REVIEW_COMPLETED?
                    }
                ...
                if (translatedCount > 0 && (finalStatus === file_model_1.FileStatus.PROCESSING || finalStatus === file_model_1.FileStatus.PENDING || finalStatus === file_model_1.FileStatus.EXTRACTED)) {
                    finalStatus = file_model_1.FileStatus.TRANSLATING;
                }
                // Add logic here if you have a distinct REVIEWING status to set
            }
            else { // If totalSegments is 0 (empty file)
                finalStatus = file_model_1.FileStatus.COMPLETED; // Mark empty files as completed
            ...
            await file.save();
            logger_1.default.info(`[${this.serviceName}.${methodName}] Updated file ${fileId} progress to ${file.progress.percentage}% and status to ${file.status}. Counts(T/C/F): ${translatedCount}/${completedCount}/${failedSegments}`);
            // *** SEND SSE UPDATE ***
            // Log the userId right before sending the update
            logger_1.default.debug(`[${this.serviceName}.${methodName}] Preparing to send SSE update for file ${fileId} to user ${userId}`);
            (0, app_1.sendSseUpdate)(userId, 'fileProgressUpdate', {
    ...
    async getRecentProjects(userId, limit = 5) {
        ...
        try {
            const projects = await project_model_1.Project.find({
                $or: [
                    { manager: new mongoose_1.Types.ObjectId(userId) },
                    { reviewers: new mongoose_1.Types.ObjectId(userId) }
                ]
            })
            ...
            logger_1.default.error(`Error in ${this.serviceName}.${methodName} for user ${userId}:`, error);
            throw (0, errorHandler_1.handleServiceError)(error, this.serviceName, methodName, '获取近期项目');
        }
    }
}
