...
class TerminologyService {
    ...
    async createTerminology(userId, data) {
        ...
        try {
            ...
            try {
                ...
                logger_1.default.info(`[${this.serviceName}.${methodName}] Mongoose save() completed successfully for ID: ${newTerminology._id}`);
            }
            catch (saveError) {
                // Log the specific save error immediately
                logger_1.default.error(`[${this.serviceName}.${methodName}] Mongoose save() FAILED! Error:`, saveError);
            ...
            logger_1.default.error(`Error in ${this.serviceName}.${methodName} (outer catch):`, error);
     ...
     */
    async getTermsByListId(terminologyId) { throw new Error('Not implemented'); }
    /**
     * Export terminology list to CSV format
     */
    async exportTerminology(terminologyId, userId) {
        ...
        try {
            ...
            if (!terminology || !Array.isArray(terminology.terms)) {
                logger_1.default.warn(`[${this.serviceName}.${methodName}] No terms found or invalid format for terminology ${terminologyId}`);
            ...
            logger_1.default.info(`[${this.serviceName}.${methodName}] Exported ${terminology.terms.length} terms for terminology ${terminologyId}`);
            return csvContent;
        }
        catch (error) {
            logger_1.default.error(`Error in ${this.serviceName}.${methodName} for ID ${terminologyId}:`, error);
            // Re-throw using handleServiceError to ensure consistent error handling
            throw (0, errorHandler_1.handleServiceError)(error, this.serviceName, methodName, '导出术语表');
        }
    }
}
