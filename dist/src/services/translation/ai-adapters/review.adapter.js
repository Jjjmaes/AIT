    ...
    calculateTokens(text, model) {
        try {
            ...
            if (model.startsWith('gpt-4')) {
                ...
                logger_1.default.warn(`Unsupported model ${model} for tiktoken calculation, falling back to gpt-3.5-turbo encoding.`);
                encodingModel = 'gpt-3.5-turbo'; // Default fallback
            }
            const encoder = (0, tiktoken_1.encoding_for_model)(encodingModel);
            const tokens = encoder.encode(text).length;
            encoder.free(); // Free up memory
            return tokens;
        }
        catch (error) {
            logger_1.default.error('Tiktoken calculation failed, falling back to estimation', { error, model });
...
`;
    ...
    async reviewText(options) {
        ...
        try {
            ...
            if (options.checkIssueTypes?.length) {
                finalIssues = finalIssues.filter((issue) => options.checkIssueTypes?.includes(issue.type));
            }
            let finalScores = reviewResult.scores || [];
            if (options.requestedScores?.length) {
                finalScores = finalScores.filter((score) => options.requestedScores?.includes(score.type));
            }
    ...
    async executeChatCompletion(messages, options) {
        logger_1.default.error('executeChatCompletion method called on ReviewAdapter, which is not supported.');
        throw new errors_1.AppError('General chat completion is not supported by the Review adapter.', 501);
    }
    // Specific method for performing review (Ensure only one implementation exists)
    async reviewTranslation(sourceText, translatedText, reviewOptions) {
        logger_1.default.info(`ReviewAdapter: Starting review for source: "${sourceText.substring(0, 50)}..."`);
        // 1. Construct prompt
        ...
        let response;
        try {
            logger_1.default.warn('ReviewAdapter cannot directly call executeChatCompletion currently. Needs refactoring.');
            ...
            if (this.config.provider !== ai_service_types_1.AIProvider.OPENAI) { // Example check
                response = { content: null, model: completionOptions.model, error: `Review not supported for provider ${this.config.provider}` };
            }
        }
        catch (error) {
            logger_1.default.error(`ReviewAdapter: Error during AI call: ${error.message}`, error);
        ...
        if (response.error || !response.content) {
            logger_1.default.error(`ReviewAdapter: AI service returned error or empty content: ${response.error || 'Empty content'}`);
            // Ensure return type matches TempReviewResponse
        ...
        try {
            ...
            logger_1.default.error('ReviewAdapter: Failed to parse review response:', parseError);
        }
        logger_1.default.info(`ReviewAdapter: Review completed. Score: ${score}`);
