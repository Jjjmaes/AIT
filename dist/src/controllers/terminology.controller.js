...
const errorHandler_1 = require("../utils/errorHandler"); // Import validateId
...
class TerminologyController {
    constructor() {
        ...
        this.getTermsByListId = async (req, res, next) => {
            const { terminologyId } = req.params;
            logger_1.default.info(`Entering TerminologyController.getTermsByListId for ID: ${terminologyId}`);
            try {
                (0, errorHandler_1.validateId)(terminologyId, 'Terminology List'); // Assuming validateId utility
                // Assuming a service method exists to fetch terms by list ID
                const terms = await terminology_service_1.terminologyService.getTermsByListId(terminologyId);
                res.status(200).json({ success: true, data: { terms } }); // Assuming standard response format
            }
            catch (error) {
                logger_1.default.error(`Error in TerminologyController.getTermsByListId for ID ${terminologyId}:`, error);
                next(error);
            }
        };
    }
    ...
    async create(req, res, next) {
        ...
        try {
            ...
            logger_1.default.error(`Error in TerminologyController.${methodName}:`, error);
            next(error);
        }
    }
    async getAll(req, res, next) {
        ...
        try {
            ...
            const result = await terminology_service_1.terminologyService.getTerminologies({ ...filters, limit: Number(limit), page: Number(page) });
    ...
    async delete(req, res, next) {
        ...
        try {
            ...
            logger_1.default.error(`Error in TerminologyController.${methodName} for ID ${req.params.terminologyId}:`, error);
            next(error);
        }
    }
    ...
    async exportById(req, res, next) {
        ...
        try {
            ...
            if (!terminologyId)
                return next(new errors_1.AppError('Terminology ID is required', 400));
            logger_1.default.info(`User ${userId} requesting export for terminology ${terminologyId}`);
            // Assume service returns CSV content or throws error
            const csvData = await terminology_service_1.terminologyService.exportTerminology(terminologyId, userId);
