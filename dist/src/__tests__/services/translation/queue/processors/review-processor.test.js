...
const review_processor_1 = require("../../../../../services/translation/queue/processors/review-processor");
...
describe('ReviewTaskProcessor', () => {
    ...
    function createMockTask(taskType, data = {}, status = queue_task_interface_1.QueueTaskStatus.PENDING) {
        return {
            id: 'task-123',
    ...
    beforeEach(() => {
        ...
        mongoose_1.default.isValidObjectId.mockReturnValue(true);
        ...
        processor = new review_processor_1.ReviewTaskProcessor();
    });
    describe('process method', () => {
        it('should handle unknown task types', async () => {
            ...
            // Act & Assert
            await expect(processor.process(mockTask)).rejects.toThrow('Unsupported review task type: unknownType');
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Review task processing failed'), expect.any(Object));
        });
        it('should process segment review tasks', async () => {
            ...
            expect(aiReviewService.reviewTranslation).toHaveBeenCalledWith('Original content', 'Translated content', expect.objectContaining({
                sourceLanguage: 'en',
                targetLanguage: 'zh-CN',
                model: 'gpt-4'
            }));
        ...
        it('should handle segment not found error in segment review tasks', async () => {
            ...
            // Act & Assert
            await expect(processor.process(mockTask)).rejects.toThrow('Segment not found: nonexistent-segment');
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Review task processing failed'), expect.any(Object));
        });
        it('should process batch segment review tasks', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Review task'), expect.any(Object));
        });
        ...
        it('should handle file not found error in file review tasks', async () => {
            ...
            // Act & Assert
            await expect(processor.process(mockTask)).rejects.toThrow('File not found: nonexistent-file');
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Review task processing failed'), expect.any(Object));
        });
        it('should process direct text review tasks', async () => {
            ...
            const result = await processor.process(mockTask);
            // Assert
            expect(aiReviewService.reviewTranslation).toHaveBeenCalledWith('Original text', 'Translated text', expect.any(Object));
            expect(result).toEqual(expect.objectContaining({
                suggestedTranslation: 'Suggested translation',
        ...
        it('should handle AI review service errors', async () => {
            ...
            // Act & Assert
            await expect(processor.process(mockTask)).rejects.toThrow('AI review service error');
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Error reviewing segment'), expect.any(Object));
        });
    });
});
