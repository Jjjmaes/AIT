...
class TranslationService {
    ...
    async translateMultipleSegments(segments, options) {
        ...
        try {
            ...
            // 如果没有使用队列，直接执行翻译
            const adapter = this.aiServiceFactory.createAdapter(this.config);
            const translationPromises = segments.map(segment => adapter.translateText(segment, options));
    ...
    async getModelInfo(modelId) {
        ...
        try {
            // 检查缓存
            if (this.cacheService) {
                const cacheKey = `${cache_keys_enum_1.CacheKey.MODEL_INFO}:${modelId}`;
                const cachedResult = await this.cacheService.get(cacheKey);
                if (cachedResult) {
                    await this.performanceMonitor.recordCacheAccess(true);
                    return cachedResult;
            ...
            await this.performanceMonitor.recordRequest(false, Date.now() - startTime);
            logger_1.default.error('Failed to get model info', {
                provider: this.config.provider,
                modelId,
                error: error instanceof Error ? error.message : 'Unknown error'
            });
    ...
    async getPricing(modelId) {
        ...
        try {
            // 检查缓存
            if (this.cacheService) {
                const cacheKey = `${cache_keys_enum_1.CacheKey.PRICING_INFO}:${modelId}`;
                const cachedResult = await this.cacheService.get(cacheKey);
                if (cachedResult) {
                    await this.performanceMonitor.recordCacheAccess(true);
                    return cachedResult;
            ...
            await this.performanceMonitor.recordRequest(false, Date.now() - startTime);
            logger_1.default.error('Failed to get pricing', {
                provider: this.config.provider,
                modelId,
                error: error instanceof Error ? error.message : 'Unknown error'
            });
