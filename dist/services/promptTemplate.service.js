...
class PromptTemplateService {
    ...
    async getTemplates(userId, filters = {}) {
        ...
        logger_1.default.info(`[${methodName}] User ID validation skipped (debugging): ${userId}`); // Log after validation
        logger_1.default.info(`[${methodName}] Service called by user: ${userId}, Filters: ${JSON.stringify(filters)}`);
        // Change log level and add log inside try
        logger_1.default.info(`[${methodName}] About to enter main try block...`); // Changed to info
        try {
            logger_1.default.info(`[${methodName}] Entered main try block.`); // Added log inside try
            const query = {};
            ...
            logger_1.default.debug(`[${methodName}] Final MongoDB Query: ${JSON.stringify(finalQuery)}`); // Log the final query
            ...
            logger_1.default.debug(`[${methodName}] Found ${total} total matching templates.`); // Log total count found
            logger_1.default.debug(`[${methodName}] Returning ${templates.length} templates for page ${page}.`); // Log count for the page
