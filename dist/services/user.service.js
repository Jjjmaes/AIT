...
class UserService {
    ...
    async getUsers(options = {}) {
        ...
        try {
            ...
            const users = await user_model_1.default.find(query).select('-password').exec();
            logger_1.default.info(`Fetched ${users.length} users with filter: ${JSON.stringify(options)}`);
            return users;
        }
        catch (error) {
            logger_1.default.error(`Error in ${this.serviceName}.${methodName}:`, error);
