 ...
 * @returns React节点数组
 */
const markupToReact = (text, handlers) => {
    // 如果没有文本或处理器，直接返回原文本
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        // 使用对应处理器处理内容并添加到结果
        if (handlers[tagName]) {
            result.push(react_1.default.createElement(react_1.default.Fragment, { key: `markup-${keyCounter++}` }, handlers[tagName](content)));
        }
        else {
            // 如果没有找到处理器，添加原始标记文本
            result.push(match[0]);
...
exports.markupToReact = markupToReact;
