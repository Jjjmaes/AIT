...
describe('ProjectTranslationService', () => {
  let projectTranslationService: ProjectTranslationService;
  ...
  beforeEach(() => {
    ...
    (FileTranslationService as jest.Mock).mockImplementation(() => mockFileTranslationService);
    (TranslationService as jest.Mock).mockImplementation(() => mockTranslationService);

    projectTranslationService = new ProjectTranslationService({
  ...
  describe('initializeProject', () => {
    it('should initialize a project translation task', async () => {
      ...
      const projectId = await projectTranslationService.initializeProject(
        'Test Project',
        'Test Description',
        files,
        options
    ...
    it('should handle initialization errors', async () => {
      ...
      await expect(
        projectTranslationService.initializeProject('Test Project', 'Test Description', files, options)
  ...
  describe('translateProject', () => {
    ...
    it('should handle translation errors', async () => {
      ...
      expect(result.files[0].status).toBe(TranslationStatus.FAILED);
    });
  });
    ...
    it('should cancel all ongoing translations', async () => {
      ...
      if (task) {
        task.files.forEach(file => {
          file.status = TranslationStatus.PROCESSING;
        });
      }
