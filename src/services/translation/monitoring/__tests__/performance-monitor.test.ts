...
describe('PerformanceMonitor', () => {
  ...
  beforeEach(() => {
    ...
    monitor = new PerformanceMonitor(mockCacheService);
  });
  ...
  describe('recordRequest', () => {
    it('should record successful request', async () => {
      ...
      expect(mockCacheService.set).toHaveBeenCalledWith(CacheKey.PERFORMANCE_METRICS, expect.any(Object));
    });
  ...
  describe('recordTaskCompletion', () => {
    it('should record successful task completion', async () => {
      await monitor.recordTaskCompletion(QueueTaskType.TRANSLATION, true, 100);
      const metrics = monitor.getMetrics();
    ...
    it('should record failed task completion', async () => {
      await monitor.recordTaskCompletion(QueueTaskType.TRANSLATION, false, 200);
      const metrics = monitor.getMetrics();
      const taskMetrics = monitor.getTaskMetrics();
    ...
    it('should calculate correct success rate for multiple tasks', async () => {
      await monitor.recordTaskCompletion(QueueTaskType.TRANSLATION, true, 100);
      await monitor.recordTaskCompletion(QueueTaskType.TRANSLATION, true, 200);
      await monitor.recordTaskCompletion(QueueTaskType.TRANSLATION, false, 300);
  ...
  describe('getTaskMetrics', () => {
    it('should return metrics for all task types', () => {
      ...
      expect(taskMetrics.length).toBe(Object.keys(QueueTaskType).length);
      expect(taskMetrics.every(metric => 
        metric.count === 0 && 
